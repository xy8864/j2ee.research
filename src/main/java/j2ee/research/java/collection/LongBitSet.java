...
public class LongBitSet {
	 ...
	 * {@code VALUE_BITS} for your needs. In most cases it is ok to keep 1M - 64M
	 * values in a bit set, so each bit set will occupy 128Kb - 8Mb.
	 */
	...
	public void clear(final long fromIndex, final long toIndex) {
		...
		if(fromSet != null) fromSet.clear(getPos(fromIndex),fromSet.length());
		// clean right subset from 0 to given index. Note that both checks are
		// independent
		if(toSet != null) toSet.clear(0,getPos(toIndex));
	}
	...
	public void forEach(final LongProcedure proc) {
		for(final Map.Entry<Long, BitSet> entry:m_sets.entrySet()){
			...
			for(int i=bs.nextSetBit(0);i >= 0;i=bs.nextSetBit(i + 1)){
				if(!proc.forEntry(baseIndex + i)) return;
			}
		}
	}
}