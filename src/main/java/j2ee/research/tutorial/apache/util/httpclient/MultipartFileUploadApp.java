...
public class MultipartFileUploadApp {
	...
	public static class MultipartFileUploadFrame extends JFrame {
		...
		private static final long	serialVersionUID	=-976542351083138610L;
		private File					targetFile;
		private JTextArea				taTextResponse;
		private DefaultComboBoxModel	cmbURLModel;

		public MultipartFileUploadFrame() {
			String[] aURLs={ "http://localhost:8080/httpclienttest/fileupload" };
			...
			btnDoUpload.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent ae) {
					...
					if(!targetURL.equals(cmbURLModel.getElementAt(cmbURL.getSelectedIndex()))){
					...
					try{
						...
						if(status == HttpStatus.SC_OK){
							appendMessage("Upload complete, response=" + filePost.getResponseBodyAsString());
						}else{
							appendMessage("Upload failed, response=" + HttpStatus.getStatusText(status));
						}
					}catch(Exception ex){
